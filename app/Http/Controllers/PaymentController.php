...
class PaymentController extends Controller
{
    ...
    { 
        if($data->status==0)
        {
            ...
            $tlog['des'] = 'Purchased STO via'.$data->gateway->name;
            $tlog['trxid'] = str_random(16);
            Transection::create($tlog);

            levelCommision($user->id, $data->price);

            $msg =  'Purchase Successful';
            send_email($user->email, $user->username, 'Purchase Successful', $msg);
